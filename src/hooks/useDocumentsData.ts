...
export const useDocumentsData = () => {
  ...
  console.log("useDocumentsData - hrDocuments:", hrDocuments?.length || 0);
  console.log("useDocumentsData - contracts:", contracts?.length || 0);
  ...
  const formattedDocuments = useMemo(() => {
    ...
    return hrDocuments.map(document => {
      try {
        ...
        try {
          ...
          if (document.uploadDate && typeof document.uploadDate === 'object' && 'seconds' in document.uploadDate) {
            ...
            if (dateStr && typeof dateStr === 'string') {
              formattedUploadDate = formatDate(new Date(dateStr));
        ...
        console.error("Error processing document", error, document);
        // Return a minimal valid document object in case of error
        return {
          id: document.id || `error-${Date.now()}`,
